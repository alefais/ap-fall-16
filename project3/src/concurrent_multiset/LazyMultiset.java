...
/**
 * The lazy implementation of a concurrent multiset.
 * @param <T> the type of the elements that can be inserted in the multiset
 *
 * @author Alessandra Fais
 */
public class LazyMultiset<T> implements ConcurrentMultiset<T> {

    /**
     * The class that defines the entry of a lazy multiset.
    ...
    private static final class MultisetElement<T> {
        ...
        /**
         * Checks if the current element is marked.
    ...
    /**
     * Constructor of the lazy multiset.
     ...
     *
     * <p>The two entries <code>pre</code> and <code>cur</code> are not locked during the search phase.</p>
    ...
    /**
     * Checks the correctness of the locked nodes: the first locked node <code>pre</code> has to
     * be reachable from the <code>head</code> and has to point to the second locked node <code>cur</code>,
     * moreover <code>pre</code> has not to be marked and <code>cur</code> has to be the searched element or
     * the end of the multiset (the tail sentinel).
     * @param value the element to be found
     * @param pre the first locked node
     * @param cur the second locked node
     * @return true if the validation succeeds, false otherwise
     */
    private boolean validate(Object value, MultisetElement<T> pre, MultisetElement<T> cur) {
        return (!pre.isMarked() && (cur == pre.getNext()) && (cur.isSentinel() || value.equals(cur.getValue())));
    }
    ...
    public int count(Object element) throws NullValueException {
        ...
        MultisetElement<T> cur = search(element).getNext();
        int count = 0;
     ...
     *
     * <p>The two entries <code>pre</code> and <code>cur</code> are locked after the search phase. The validation phase
     * checks their correctness: if it fails the locks are released and it is necessary to start again, else the
     * procedure is like that of the fine grain synchronization {@link FineGrainMultiset#add(Object, int)}.</p>
     *
     ...
     * entry (or when the occurrences are updated), in the unsuccessful case when the validation is completed
     * successfully but the call is unsuccessful (exception raised).</p>
     * @param element the element to be inserted in the multiset
    ...
    public int add(T element, int occurrences) throws NullValueException, InvalidCountException, FullMultisetException {
        ...
        while (!valid) {
            try {
                ...
                valid = validate(element, pre, cur);
                if (valid) {
     ...
     * <p>The two entries <code>pre</code> and <code>cur</code> are locked after the search phase. The validation phase
     * checks their correctness: if it fails the locks are released and it is necessary to start again, else the
     * procedure is like that of the fine grain synchronization {@link FineGrainMultiset#remove(Object, int)}.</p>
     *
     * <p>At the beginning it is checked if the multiset is in <b>read only mode</b>: this happens when the
     * {@link #removeAll(Collection)} is executed on the multiset. The strategy is to wait until the end of the
     * execution of the {@link #removeAll(Collection)} in order to work on the updated multiset and to avoid losing
     ...
     * marked (or when the occurrences are updated), in the unsuccessful case when the validation
     * is completed successfully but the call is unsuccessful (exception raised or element not found).</p>
     * @param element the element to be removed from the multiset
     ...
     * have already been logically removed.</p>
